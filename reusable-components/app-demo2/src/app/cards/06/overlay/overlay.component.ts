...
import { AnimationBuilder, AnimationTriggerMetadata, AnimationStateMetadata } from '@angular/animations';
...
export class Overlay6Component implements AfterViewInit {
  ...
  @Input() overlayActivatedAnimation: AnimationTriggerMetadata;
  @Input() overlayDeactivatedAnimation: AnimationTriggerMetadata;
  ...
  ngAfterViewInit(): void {
    ...
    if (this.overlayActivatedAnimation == null
      ...
      Object.keys(state.styles.styles).forEach(key => {
        this.renderer.setStyle(el, key, state.styles.styles[key]);
      });
  }
